...
def graphDCNMI(S1, S2):
    ...
    G1, G2 = graph_Gset(S1), graph_Gset(S2)
    ...
    for i in node_order[:int(eps * N)]:
        for neig in adjlist[i]:
            if neig > i:
                repeated = True
                while repeated == True:
                    #to_add = (i, random.randint(0, N-1))
                    to_add = tuple(sorted([i, i]))  # initialize to (i, i) to enter the while loop -- this made the KeyError much rarer
                    while to_add[1] == i:
...
def typeIII(Gset, partition, eps):
    ...
    for rs in edges.keys():
        ...
        #while count < rand_rs: # add the timeout below
