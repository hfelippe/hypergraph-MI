...
def project_subsets(e, l):
    ...
    if isinstance(e, tuple): # if e is a single tuple, wrap it in a list to handle it uniformly
        e = [e]
    return {combo for edge in e for combo in itertools.combinations(sorted(edge), l)} # sorted garantees order
...
def NMI(N,e1,e2):
    ...
    H1,H2 = entropy([p1,1-p1]), ent([p2,1-p2])
...
def hNMI_project(N,e1,e2,l):
    ...
    proj_e1=project_subsets(e1,d) # get the projected hyperedge set of e1
    Ncl = binom(N,l)
